...
Wordmark, utility nav, availability chip, kinetic headline, project tile, filter rail, editorial list, service index, profile card, experience timeline, testimonial grid, proof strip, process timeline, modal reel, multi-step brief, field system, notice, footer, and owner tables.

The About & Experience system pairs a high-contrast profile card with a structured timeline for workplace, role, period, location, and responsibilities. The same managed history is reused on the About and résumé pages. Testimonials use a large editorial carousel with progress, direct navigation, play/pause controls, swipe support, and gentle six-second auto-advance when approved entries exist. It pauses for interaction and respects reduced motion. An animated but clearly labelled no-fabrication state appears while the collection is empty.
