...
function inferMediaType(url: string): "image" | "video" {
  return /\/video\/upload\/|\.(mp4|webm|mov|m4v)(?:$|[?#])/i.test(url) ? "video" : "image";
}
...
export function AdminTestimonialsForm({ initial }: { initial: SiteContent }) {
  ...
  function updateMediaUrl(id: string, url: string) {
    setContent((current) => ({ ...current, testimonials: current.testimonials.map((item) => item.id === id ? { ...item, mediaUrl: url, mediaType: url && item.mediaType === "none" ? inferMediaType(url) : item.mediaType } : item) }));
  }

  async function persist(nextContent: SiteContent) {
    const response = await fetch("/api/admin/content", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(nextContent) });
  ...
  async function uploadMedia(id: string, file: File | undefined) {
    ...
    try {
      ...
      const nextContent: SiteContent = { ...content, testimonials: content.testimonials.map((item) => item.id === id ? { ...item, mediaType: uploaded.resource_type as "image" | "video", mediaUrl: uploaded.secure_url ?? "", mediaAlt: item.mediaAlt || `${item.name || "Client"} testimonial ${uploaded.resource_type}` } : item) };
      setContent(nextContent);
      try {
        await persist(nextContent);
        setSaved(true); setMessage("Client media uploaded and published.");
      } catch {
        setSaved(false); setMessage("The media uploaded successfully. Complete the required testimonial fields, then choose Save testimonials to publish it.");
      }
    } catch (error) {
